'''
Desafio
Um supermercado está fazendo uma promoção de venda de refrigerantes. Se um dia você comprar refrigerantes 
e levar os cascos vazios no dia seguinte, ela troca cada conjunto de K garrafas vazias  por uma garrafa cheia.
Um cliente quer aproveitar ao máximo essa oferta e por isso comprou várias garrafas no primeiro dia da 
promoção. Agora ele quer saber quantas garrafas terá ao final do segundo dia da promoção, se usá-la ao máximo.
...
Em cada uma das T linhas a seguir vêm dois inteiros N e K (1 ≤ K, N ≤ 10000),  respectivamente o número de 
refrigerantes comprados e o número de garrafas vazias para ganhar uma cheia.

Saída
Para cada caso de teste imprima o número de garrafas que o cliente terá no segundo dia, se aproveitar ao 
máximo a oferta.
'''
...
for i in range(T):
  ...
  #observação importante para os desafios: não importa o nome das variáveis e como chega ao resultado. 
  #Basta criar uma váriável que receba o input na sequência explicada no enunciado.
  #Neste, por exemplo, temos 3 inputs na sequência:
  #primeiro input: T 
  #segundo input: N ou qualquer nome que desejar
  #terceiro input: K ou qualquer nome que desejar
